...
void setupRazor() {
  println("Trying to setup and synch CubeSat...");
...
void draw() {
  ...
  if (!synched) {
    textAlign(CENTER);
    fill(255);
    text("Connecting to AGS...", width/2, height/2, -200);
  ...
  while (serial.available() >= 12) {
    if(first)
    {
      inityaw = readFloat(serial);
      initpitch = yaw = readFloat(serial);
      initroll = readFloat(serial);
      first = false;
    }
    yaw = readFloat(serial);//- inityaw;
    pitch = readFloat(serial) - initpitch;
    roll = readFloat(serial) - initroll;
  }
  ...
  // Output info text
  text("Press 'a' to align cubeSat", 10, 25);
