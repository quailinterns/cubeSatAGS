...
#define GYRO_ADDRESS  ((int) 0x69) // 0x68 = 0xD0 / 2
...
boolean i2c = false;
const int xPin = 0;  //Analog read pins for the accel, since it is not using I2C 
const int yPin = 1;
const int zPin = 2;
...
void Read_Accel()
{
  ...
  {
    //following is accel code, and has been motified for our current setup
  
    int xRead = (int) analogRead(xPin);  //read the analog values
    int yRead = (int) analogRead(yPin);
    int zRead = (int) analogRead(zPin);
...
// 9DOF Razor IMU SEN-10736 using HMC5883L magnetometer
    ...
    magnetom[0] = 0 * ((((int) buff[4]) << 8) | buff[5]);  // X axis (internal sensor -y axis)
    magnetom[1] = 0 * ((((int) buff[0]) << 8) | buff[1]);  // Y axis (internal sensor -x axis)
    magnetom[2] = 0 * ((((int) buff[2]) << 8) | buff[3]);  // Z axis (internal sensor -z axis)
